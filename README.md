...
Mock
Used for testing the UI and backend without any model.

Ollama
Uses a local large language model running on the developer’s machine. This allows real AI responses with no API costs and no internet dependency.

OpenAI (optional)
Can be enabled later for production-grade hosted models without changing any frontend code.
